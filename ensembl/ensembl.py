...
class EnsemblRestClient(object):
    ...
    def login(self, username, password):
        # Ensembl login endpoint
        login_endpoint = 'https://rest.ensembl.org/login'
        ...
        request = Request(login_endpoint, headers=headers)
        ...
        try:
            ...
            if content:
                ...
                print("Login successful. Auth token acquired.")
            else:
                print("Failed to acquire auth token.")
        except HTTPError as e:
            sys.stderr.write('Login failed: Status code: {0.code} Reason: {0.reason}\n'.format(e))
...
def run(species, symbol):
    ...
    username = os.getenv('ENSEMBL_USERNAME')
    password = os.getenv('ENSEMBL_PASSWORD')
